    ...
    db
        ...
        .then((documentRef) => {
            ...
            documentRef.get().then(async documentSnapshot => {
                ...
                const publicUrl = generateQR(documentRef.id, `Restaurants/${documentSnapshot.data().slug}/qr_${documentRef.id}-${new Date().getTime()}.png`);
...
exports.createDeal = (request, response) => {
    ...
    const expiresAt = createdAt.set('hour', expiryTimeParts[0]).set('minutes', expiryTimeParts[1]);

    let startTimeParts = request.body.startsAt 
        ? request.body.startsAt 
        : createdAt.format('hh:mm');
    startTimeParts = startTimeParts.split(':');
    const startsAt = createdAt.set('hour', startTimeParts[0]).set('minutes', startTimeParts[1]);
...
exports.getDeals = async (request, response) => {
    // Build query
    let collectionReference = db.collection(`Deals`)
        .where('restaurantId', '==', request.user.restaurantId)

    // Apply filters
    if(request.query.active != undefined){
        collectionReference = collectionReference.where('active', '==', request.query.active == 'true' || false)
    }
...
exports.getDeal = async (request, response) => {
    ...
    if (docSnap.exists) {
        return response.json({
            id: docSnap.id,
            ...docSnap.data(),
            startsAt: dayjs.unix(docSnap.data().startsAt.seconds).utcOffset(UTC_OFFSET).format('HH:mm'),
            expiresAt: dayjs.unix(docSnap.data().expiresAt.seconds).utcOffset(UTC_OFFSET).format('HH:mm'),
            createdAt: dayjs.unix(docSnap.data().createdAt.seconds).utcOffset(UTC_OFFSET)
        });
    } else {
        return response.status(204).json({
...
exports.updateDeal = async (request, response) => {
    ...
    if(updateObject.startsAt){
        const currentStartTime = dayjs.unix(deal.data().startsAt.seconds).utc().utcOffset(UTC_OFFSET);
        let startTimeParts = updateObject.startsAt.split(':');
        const startsAt = currentStartTime.set('hour', startTimeParts[0]).set('minutes', startTimeParts[1]);
    ...
    if(updateObject.expiresAt){
        const currentExpiryTime = dayjs.unix(deal.data().expiresAt.seconds).utc().utcOffset(UTC_OFFSET);
        let expiryTimeParts = updateObject.expiresAt.split(':');
        const expiresAt = currentExpiryTime.set('hour', expiryTimeParts[0]).set('minutes', expiryTimeParts[1]);

        // ToDo: Validate date or return error

        // Overwrite original data
        app.firestore.Tim
        updateObject.expiresAt = app.firestore.Timestamp.fromDate(expiresAt.toDate());
    }
    ...
    .then(() => {
        docRef.get().then(documentSnapshot => {
            response.json({
                ...
                startsAt: dayjs.unix(documentSnapshot.data().startsAt.seconds).utcOffset(UTC_OFFSET).format('HH:mm'),
                expiresAt: dayjs.unix(documentSnapshot.data().expiresAt.seconds).utcOffset(UTC_OFFSET).format('HH:mm'),
                createdAt: dayjs.unix(documentSnapshot.data().createdAt.seconds).utcOffset(UTC_OFFSET)
            });
        });
    })
    .catch((err) => {
...
exports.getReservationsList = async (request, response) => {
    ...
    if(range_init != undefined){
        range_init = dayjs(request.query.range_init).isValid()
            ? dayjs(dayjs(request.query.range_init).toISOString()).utcOffset(UTC_OFFSET, true).toDate()
            : dayjs(todayDate).utcOffset(UTC_OFFSET, true).toDate();
    } else {
        range_init = todayDate
            .set('date', 1)
            .hour(0).minute(0).second(0)
            .utcOffset(UTC_OFFSET, true).toDate()
    }
    if(range_end){
        range_end = dayjs(request.query.range_end).isValid()
            ? dayjs(request.query.range_end).hour(23).minute(59).second(59).utcOffset(UTC_OFFSET, true).toDate()
    ...
    collectionReference = collectionReference
        .where('reservationDate', '>=', app.firestore.Timestamp.fromDate(range_init))
    ...
    if (collection.size > 0) {
        ...
        for(doc of collection.docs){
            console.log(doc.id, dayjs.unix(doc.data().reservationDate.seconds).toDate(),  dayjs(range_end).toDate(), doc.data().reservationDate <= app.firestore.Timestamp.fromDate(range_end))
            // Get Deal
            ...
            deals.push({
                ...
                reservationDate: dayjs.unix(doc.data().reservationDate.seconds).toDate(),
...
exports.createQR = async (request, response) => {
    const publicUrl = await generateQR(
        ...
        `Restaurants/${request.body.restaurantSlug}/qr_${request.body.restaurantId}-${new Date().getTime()}.png`
    );
...
const generateQR = async (restaurantId, path) => {
    ...
    // generate QR code
    var QRCode = require('qrcode')
    const qrCode = await QRCode.toDataURL(restaurantId, {scale: 20, color: {dark: '#E53E3A'}})
